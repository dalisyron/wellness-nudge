...
if [ "$STARTED" = true ]; then
  ...
  echo ""
  echo "  GET messages:  curl -s $MIMOE_URL/api/$MIM_NAME/v1/messages -H 'Authorization: Bearer $MIM_API_KEY'"
  echo ""
  echo "Try it:"
  echo ""
  echo "  curl -s $MIMOE_URL/api/$MIM_NAME/v1/messages -H 'Authorization: Bearer $MIM_API_KEY' | python3 -m json.tool"
else
